...
RfxCom.prototype.open = function () {
  ...
  self.serialport.on("data", function (data) {
    if (self.options.debug) {
      self.logger.debug("Received", { data: self.dumpHex(data) });
    }
    ...
    if (typeof handler !== "undefined") {
      ...
      if (self.options.debug) {
        self.logger.debug("Unhandled packet type", { packetType: self.dumpHex([packetType]) });
      }
    }
  });

  self.serialport.on("error", function (msg) {
    self.logger.error("serialport", { diagnostic: msg });
  });

  self.serialport.on("end", function () {
    self.logger.debug("serialport end");
  });

  self.serialport.on("drain", function () {
    self.logger.debug("serialport drain");
  });
};
